...
void legitbot_weapon_classes(int& current_players_esp_subtab, int tab_amount, Vector _pos, int MenuAlpha_Main)
{
	int in_sizew_esp_player_subtabs = GroupBoxSize_Width - 8;
	static std::string ESP_Player_SubTabs_Names[6] = { "Pistols", "Rifles", "Snipers", "Shotgun", "SMG", "Others" };
...
{
	int in_sizew_esp_player_subtabs = (375 / 1.5) - 8;
	static std::string ESP_Player_SubTabs_Names[1] = {"Body-Aim"};
...
{
	int in_sizew_esp_player_subtabs = GroupBoxSize_Width - 8;
	static std::string ESP_Player_SubTabs_Names[2] = {"Aimbot"};
...
{
	int in_sizew_esp_player_subtabs = GroupBoxSize_Width - 8;
	static std::string ESP_Player_SubTabs_Names[2] = { "Checks" };
...
void Save_Config()
{
	...
	c_event_logs::get().add(string_to_add, Color(c_config::get().menu_color_r, c_config::get().menu_color_g, c_config::get().menu_color_b, 255));
}
...
void Menu::Render()
{
	...
	RECT TitleText_Size = g_pSurface->GetTextSizeRect(Globals::CourierNew, "xy0 repaste");

	g_pSurface->DrawT(_pos.x + 3, _pos.y + (20 / 2) - (TitleText_Size.bottom / 2), Color(143, 143, 143, MenuAlpha_Text), Globals::CourierNew, false, "xy0 repaste");
	...
	static const int tab_amount = 7;
	static std::string tab_names[tab_amount] = { "Legit", "Rage", "AA", "Visuals", "Misc", "Skins", "Players" };
	...
	if (tab_selected == 0)
	{
		...
		groupbox(5, 52, GroupBoxSize_Width, (GroupBoxSize_Height * 1.334) + 5, "test", false, true);
		...
		slider(100, "RCS X", &c_config::get().pistol_rcs_x, "", 1);
		slider(100, "RCS Y", &c_config::get().pistol_rcs_y, "", 1);
		DrawLegitMain_Subtab(aimbot_main_subtab, 1, _pos, MenuAlpha_Main);
		checkbox("Enable", &c_config::get().legit_aimbot_enabled);
		checkbox("On Key", &c_config::get().legitbot_onkey);
		keybind(&c_config::get().legitbotkey, "legitbot_key_hahah_lol_edgy");
		checkbox("Friendly Fire", &c_config::get().FriendlyFireLegit);
		checkbox("Silent Aim", &c_config::get().LegitSilentAim);
		checkbox("Auto Pistol", &c_config::get().autopistol);
		checkbox("Backtrack", &c_config::get().legit_aimbot_backtrack);*/
		checkbox("Disabled for now", &c_config::get().scope_check);

		/*static int legitbot_selected_weapon = 0;
		groupbox(TopLeftGroupBox_X + GroupBoxSize_Width + 5, 52, GroupBoxSize_Width, (GroupBoxSize_Height * 1.334) + 5, "test", false,
			true);
		legitbot_weapon_classes(legitbot_selected_weapon, 6, _pos, MenuAlpha_Main);
		switch (legitbot_selected_weapon)
		{
		case 0:
		{
			slider(100, "FoV", &c_config::get().pistol_fov, "degree", 0.1f);
			slider(100, "Smooth", &c_config::get().pistol_smooth, "%", 0.1f);
			checkbox("Legit RCS", &c_config::get().rcs);
			if (c_config::get().rcs) {
				slider(100, "RCS X Factor", &c_config::get().rcs_x, "%", 1);
				slider(100, "RCS Y Factor", &c_config::get().rcs_y, "%", 1);
			}
		...
		case 1:
		{
			slider(100, "FoV", &c_config::get().rifle_fov, "degree", 0.1f);
			slider(100, "Smooth", &c_config::get().rifle_smooth, "%", 0.1f);
			checkbox("Legit RCS", &c_config::get().rcs);
			if (c_config::get().rcs) {
				slider(100, "RCS X Factor", &c_config::get().rcs_x, "%", 1);
				slider(100, "RCS Y Factor", &c_config::get().rcs_y, "%", 1);
		...
		case 2:
		{
			slider(100, "FoV", &c_config::get().sniper_fov, "degree", 0.1f);
			slider(100, "Smooth", &c_config::get().sniper_smooth, "%", 0.1f);
			checkbox("Legit RCS", &c_config::get().rcs);
			if (c_config::get().rcs) {
				slider(100, "RCS X Factor", &c_config::get().rcs_x, "%", 1);
				slider(100, "RCS Y Factor", &c_config::get().rcs_y, "%", 1);
		...
		case 3:
		{
			slider(100, "FoV", &c_config::get().shotgun_fov, "degree", 0.1f);
			slider(100, "Smooth", &c_config::get().shotgun_smooth, "%", 0.1f);
			checkbox("Legit RCS", &c_config::get().rcs);
			if (c_config::get().rcs) {
				slider(100, "RCS X Factor", &c_config::get().rcs_x, "%", 1);
				slider(100, "RCS Y Factor", &c_config::get().rcs_y, "%", 1);
		...
		case 4:
		{
			slider(100, "FoV", &c_config::get().smg_fov, "degree", 0.1f);
			slider(100, "Smooth", &c_config::get().smg_smooth, "%", 0.1f);
			checkbox("Legit RCS", &c_config::get().rcs);
			if (c_config::get().rcs) {
				slider(100, "RCS X Factor", &c_config::get().rcs_x, "%", 1);
				slider(100, "RCS Y Factor", &c_config::get().rcs_y, "%", 1);
		...
		case 5:
		{
			slider(100, "FoV", &c_config::get().legit_aimbot_fov, "degree", 0.1f);
			slider(100, "Smooth", &c_config::get().legitbotSmooth, "%", 0.1f);
			checkbox("Legit RCS", &c_config::get().rcs);
			if (c_config::get().rcs) {
				slider(100, "RCS X Factor", &c_config::get().rcs_x, "%", 1);
				slider(100, "RCS Y Factor", &c_config::get().rcs_y, "%", 1);
		...
		combobox(6, "Hitbox ", legitHitbox, &c_config::get().legitHitbox);
		*/
	}
	else if (tab_selected == 1)
	{
		groupbox(5, 52, GroupBoxSize_Width, GroupBoxSize_Height, "test", false, true);
		DrawAimbotMain_Subtab(aimbot_main_subtab, 1, _pos, MenuAlpha_Main);
		...
		DrawAimbotTargetSubTab(current_target_selection_aimbots, 1, _pos, MenuAlpha_Main);
		...
		case 0:
		{
			...
			slider(100, "Head Scale", &c_config::get().aimbot_headpointscale_auto, "%", 1);
			slider(100, "Body Scale", &c_config::get().aimbot_bodypointscale_auto, "%", 1);
		}
		break;
		case 1:
		{
			...
			slider(100, "Head Scale", &c_config::get().aimbot_headpointscale_scout, "%", 1);
			slider(100, "Body Scale", &c_config::get().aimbot_bodypointscale_scout, "%", 1);
		}
		break;
		case 2:
		{
			...
			slider(100, "Head Scale", &c_config::get().aimbot_headpointscale_awp, "%", 1);
			slider(100, "Body Scale", &c_config::get().aimbot_bodypointscale_awp, "%", 1);
		}
		break;
		case 3:
		{
			...
			slider(100, "Head Scale", &c_config::get().aimbot_headpointscale_heavy, "%", 1);
			slider(100, "Body Scale", &c_config::get().aimbot_bodypointscale_heavy, "%", 1);
		}
		break;
		case 4:
		{
			...
			slider(100, "Head Scale", &c_config::get().aimbot_headpointscale_other, "%", 1);
			slider(100, "Body Scale", &c_config::get().aimbot_bodypointscale_other, "%", 1);
		}
		break;
		}
	}
		...
		std::string pitch[19] = { "None", "Emotion", "Down", "Up", "Zero", "Jitter", "Down Jitter", "Up Jitter", "Zero Jitter", "Spin", "Up Spin", "Down Spin", "Random", "Switch", "Down Switch", "Up Switch", "Fake Up", "Fake Down", "Custom" };
		std::string yaw[7] = { "Disabled", "Backwards", "Origin", "Spin", "Manual", "Random", "Custom" };
		std::string yaw_add[5] = { "None", "Jitter", "Switch", "Spin", "Random" };
		std::string on_shot[2] = { "Stable", "Experimental" };
		std::string desync[8] = { "Disabled", "Ephemeral", "Cromulent", "Ornery", "Jitter", "180", "58", "Real + 180 + Random(45)" };
		std::string freestand_mode[3] = { "Threat", "Crosshair", "Distance" };
		switch (current_subtab_antiaim_selection)
		{
		case 0:
		{
			combobox(19, "X", pitch, &c_config::get().pitch_standing);
			combobox(6, "Y", yaw, &c_config::get().yaw_standing);
			combobox(5, "Add", yaw_add, &c_config::get().yaw_standing_add);
			slider(360, "yaw add range", &c_config::get().yaw_standing_add_angle, "°", -360);
			slider(360, "custom yaw", &c_config::get().custom_yaw_stand, "°", -360);
			slider(89, "custom pitch", &c_config::get().custom_pitch_stand, "°", -89);
			slider(100, "spin_speed", &c_config::get().spin_speed_stand, "%", 1);
		}
		break;
		case 1:
		{
			combobox(19, "X", pitch, &c_config::get().pitch_moving);
			combobox(6, "Y", yaw, &c_config::get().yaw_moving);
			combobox(5, "Add", yaw_add, &c_config::get().yaw_moving_add);
			slider(360, "yaw add range", &c_config::get().yaw_moving_add_angle, "°", -360);
			slider(360, "custom yaw", &c_config::get().custom_yaw_move, "°", -360);
			slider(89, "custom pitch", &c_config::get().custom_pitch_move, "°", -89);
			slider(100, "spin_speed", &c_config::get().spin_speed_move, "%", 1);
		}
		break;
		case 2:
		{
			combobox(19, "X", pitch, &c_config::get().pitch_air);
			combobox(6, "Y", yaw, &c_config::get().yaw_air);
			combobox(5, "Add", yaw_add, &c_config::get().yaw_air_add);
			slider(360, "yaw add range", &c_config::get().yaw_air_add_angle, "°", -360);
			slider(360, "custom yaw", &c_config::get().custom_yaw_air, "°", -360);
			slider(89, "custom pitch", &c_config::get().custom_pitch_air, "°", -89);
			slider(100, "spin_speed", &c_config::get().spin_speed_air, "%", 1);
		}
		break;
		case 3:
		{
			...
			combobox(8, "Desync", desync, &c_config::get().desync);
			if (c_config::get().desync == 5){
				slider(180, "Add to desync", &c_config::get().desync_add, "°", 1);
			}
		...
		slider(20, "Fakelag", &c_config::get().fakelag, "t", 1);
		slider(100, "Fakelag Randomization", &c_config::get().variance, "%", 1);
		slider(20, "Fake Duck", &c_config::get().fakeduck_test, "t", 1);
	}
	else if (tab_selected == 3)
	{
		...
		std::string Hitmarker_Type[5] = { "Disabled", "Arena Switch", "Impact", "Bell", "Nice Headshot"};
		if (current_players_esp_subtab == 0)
		{
			...
			checkbox("Spread", &c_config::get().draw_spread);
		}
		...
		else if (current_players_esp_subtab == 2)
		{
			...
			color_selector("desync_cham_col", &c_config::get().desync_chams_color_r, &c_config::get().desync_chams_color_g,
		...
		checkbox("Render Keystrokes", &c_config::get().misc_keystrokes);
		...
		slider(100, "Nightmode intensity", &c_config::get().nightmodeintensity, "&", 1);
	...
	else if (tab_selected == 4)
	{
		...
		checkbox("Metal World", &c_config::get().metalworld);
		checkbox("Left Hand Knife", &c_config::get().lefthandknife);
		slider(100, "Slow Walk", &c_config::get().slowwalk_speed, "%", 1);
		checkbox("Clantag", &c_config::get().misc_clantag);

		groupbox(TopLeftGroupBox_X, TopLeftGroupBox_Y, GroupBoxSize_Width, GroupBoxSize_Height, "test");
		std::string KeyBindList[4] = { "Thirdperson", "Slow Walk", "Swap Manual", "Fake Duck" };
		static int CurrentKeybind = 0;

		combobox(4, "Keybinds", KeyBindList, &CurrentKeybind);
		...
		case 2: keybind(&c_config::get().manual_swap_bind, "c");
			break;
		case 3: keybind(&c_config::get().fakeduck_bind, "d");
			break;
		}
	
		groupbox(TopLeftGroupBox_X + GroupBoxSize_Width + 5, TopLeftGroupBox_Y, GroupBoxSize_Width, GroupBoxSize_Height,
			"test");
		slider(250, "Ping Spiker(real)", &c_config::get().fakelagspoof, "ms", 0);
		...
		groupbox(TopLeftGroupBox_X + GroupBoxSize_Width + 5, 52, GroupBoxSize_Width, GroupBoxSize_Height, "test");
		checkbox("Bullet Tracer", &c_config::get().bullet_tracer);
		//slider(10, "Bullet Tracer Duration", &c_config::get().bulletbeamduration, "s", 1);
		//checkbox("Damage Esp", &c_config::get().damageesp);
		//checkbox("Damage Group", &c_config::get().damagegroup);
		checkbox("Capsule", &c_config::get().capsule);
		slider(10, "Capsule Duration", &c_config::get().capsuleduration, "s", 1);
		checkbox("Render Keystrokes", &c_config::get().misc_keystrokes);
		checkbox("Viewmodel Offset Adjustment", &c_config::get().customviewmodel);
		if (c_config::get().customviewmodel) {
			slider(20, "Viewmodel X Position", &c_config::get().ViewmodelX, "%", 1);
			slider(20, "Viewmodel Y Position", &c_config::get().ViewmodelY, "%", 1);
			slider(20, "Viewmodel Z Position", &c_config::get().ViewmodelZ, "%", 1);
		}
		/*color_selector("sound_esp_color", &c_config::get().sound_color_r, &c_config::get().sound_color_g,
			&c_config::get().sound_color_b, &c_config::get().sound_color_a);
		checkbox("Sound Esp", &c_config::get().SoundEsp);*/
	}
	...
	{
		DoPlayerList(5, 50, (GroupBoxSize_Width * 2) + 5, (GroupBoxSize_Height * 2) + 5);
	}
}
