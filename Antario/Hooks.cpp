...
#include "Features\bulletbeams\bullettracers.h""
#include "Menu\Menu.h"
...
/*void DamageESP::Draw()
{
	if (c_config::get().damageesp || c_config::get().damagegroup)
		for (int i = 0; i < MAX_FLOATING_TEXTS; i++)
		{
			...
			if (Utils::WorldToScreen(txt->hitPosition, screen))
			{
				...
				if (c_config::get().damageesp)
				{
					...
					DrawStringWithFont(fntVerdana10, screen.x - width / 2, screen.y - 11, D3DCOLOR_ARGB(int(c_config::get().damageesp_color_a), int(c_config::get().damageesp_color_r), int(c_config::get().damageesp_color_g), int(c_config::get().damageesp_color_b)), msg);
				}
				if (c_config::get().damagegroup)
				{
					char msgxd[12];
					...
					DrawStringWithFont(fntVerdana10, screen.x - widthxd / 2, screen.y - 63, D3DCOLOR_ARGB(int(c_config::get().damageesp_color_a), int(c_config::get().damageesp_color_r), int(c_config::get().damageesp_color_g), int(c_config::get().damageesp_color_b)), msgxd);
				}
			}
		}
}*/
...
void soundesp::draw()
{
	...
	{
		g_soundesp.draw_circle(Color(c_config::get().sound_color_a, c_config::get().sound_color_g, c_config::get().sound_color_b, c_config::get().sound_color_a), sound_logs[i].position);
		if (c_config::get().LocalSoundEsp)
			g_soundesp.draw_circle(Color(c_config::get().local_sound_color_r, c_config::get().local_sound_color_g, c_config::get().local_sound_color_b, c_config::get().local_sound_color_a), sound_logs[i].position);
	...
	if (c_config::get().LocalSoundEsp)
	{
		...
		{
			g_soundesp.draw_circle(Color(c_config::get().sound_color_a, c_config::get().sound_color_g, c_config::get().sound_color_b, c_config::get().sound_color_a), sound_logs_local[i].position);
			if (c_config::get().LocalSoundEsp)
				g_soundesp.draw_circle(Color(c_config::get().local_sound_color_r, c_config::get().local_sound_color_g, c_config::get().local_sound_color_b, c_config::get().local_sound_color_a), sound_logs_local[i].position);
...
void soundesp::draw_circle(Color color, Vector position)
{
	...
	Beam_t* beam = g_pRenderBeams->CreateBeamRingPoint(beamInfo);
	if (beam) g_pRenderBeams->DrawBeam(beam);
}
*/
...
void Hooks::Init()
{
	...
	Globals::keystrokes = g_pSurface->FontCreate();
...
bool __fastcall Hooks::CreateMove(IClientMode* thisptr, void* edx, float sample_frametime, CUserCmd* pCmd)
{
	...
	if(Globals::LocalPlayer && Globals::LocalPlayer->IsAlive())
	{
		...
		g_LegitAimbot.OnCreateMove();
		g_LegitAimbot.CM_Backtrack();
	}
...
void __fastcall Hooks::PaintTraverse(PVOID pPanels, int edx, unsigned int vguiPanel, bool forceRepaint, bool allowForce)
{
	...
	auto scopeArc = reinterpret_cast<void(__fastcall*)(const char*)>(Utils::FindSignature("client_panorama.dll", "\x6A?\xFF\x50\x3C\x8B\x0D????\xFF\xB7 5"));
	auto scopeLens = reinterpret_cast<void(__fastcall*)(const char*)>(Utils::FindSignature("client_panorama.dll", "\xFF\x50\x3C\x8B\x4C\x24\x20 3"));
	...
	/*if (c_config::get().remove_scope && (returnAddress == memory.scopeArc || returnAddress == memory.scopeLens)) {
		a = 0;
	}*/
...
void Event::FireGameEvent(IGameEvent* event)
{
	...
	if (!strcmp(name, "player_hurt")) {
		...
		if (c_config::get().hitmarker) {
			...
			case 2: g_pSurface->PlaySoundA("physics\\metal\\metal_solid_impact_bullet2.wav"); break;
			case 3: g_pSurface->PlaySoundA("training\\bell_impact.wav"); break;
			case 4: g_pSurface->PlaySoundA("commander\\train_bodydamageheadshot_01b.wav"); break;
			case 6: g_pSurface->PlaySoundA("physics\\glass\\glass_impact_bullet1.wav"); break;
			}
			Hitmarkertime = 255;
		}
		if (c_config::get().capsule)
		{
			C_BaseEntity* hurtnibba = (C_BaseEntity*)g_pEntityList->GetClientEntity(g_pEngine->GetPlayerForUserID(event->GetInt("userid")));
			if (hurtnibba && hurtnibba->EntIndex() > 0 && hurtnibba->EntIndex() < 64)
			{
				if (Globals::LocalPlayer && hurtnibba != Globals::LocalPlayer)
					CapsuleOverlay(hurtnibba, Color(g_Math.RandomFloat(0.f, 255.f), g_Math.RandomFloat(0.f, 255.f), g_Math.RandomFloat(0.f, 255.f), 255.f), c_config::get().capsuleduration);
			}
		}
		/*if (c_config::get().SoundEsp)
		{
			auto victim = g_pEntityList->GetClientEntity(g_pEngine->GetPlayerForUserID(event->GetInt("userid")));
			if (attacker == Globals::LocalPlayer && !c_config::get().local_sound_color_r && !c_config::get().local_sound_color_g && !c_config::get().local_sound_color_b && !c_config::get().local_sound_color_a)
				sound_logs.push_back(sound_info(victim->GetAbsOrigin(), g_pGlobalVars->curtime, event->GetInt("userid")));
			else if (attacker == Globals::LocalPlayer && !c_config::get().local_sound_color_r && !c_config::get().local_sound_color_g && !c_config::get().local_sound_color_b && !c_config::get().local_sound_color_a)
		...
		if (strstr(event->GetName(), "player_footstep") || strstr(event->GetName(), "player_jump"))
		{
			...
			if (walker->GetTeam() != Globals::LocalPlayer->GetTeam())
				sound_logs.push_back(sound_info(walker->GetAbsOrigin(), g_pGlobalVars->curtime, event->GetInt("userid")));

			if (walker == Globals::LocalPlayer && c_config::get().LocalSoundEsp)
				sound_logs_local.push_back(sound_info(Globals::LocalPlayer->GetAbsOrigin(), g_pGlobalVars->curtime, event->GetInt("userid")));
		}*/
	}
...
void __fastcall Hooks::SceneEnd(void *ecx, void *edx)
{
	...
	for (int i = 1; i < g_pEngine->GetMaxClients(); ++i)
	{
		...
		*/
		...
		if (IsLocal)
		{
			if (c_config::get().desync_chams && pPlayerEntity == Globals::LocalPlayer && c_config::get().desync > 0 && c_config::get().aimbot_enabled)
			{
				Vector OldAngles = pPlayerEntity->GetEyeAngles();
				pPlayerEntity->SetAbsAngles(Vector(0, Globals::FakeAngle.y, 0.f));
				g_pRenderView->SetBlend(c_config::get().desync_chams_color_a / 100.f);
				g_pModelRender->ForcedMaterialOverride(materials.at(0));
				g_pRenderView->SetColorModulation(fake_color);
				pPlayerEntity->DrawModel(STUDIO_RENDER, 255);
				g_pModelRender->ForcedMaterialOverride(nullptr);
				pPlayerEntity->SetAbsAngles(Vector(0, OldAngles.y, 0.f));
			}
		}
		g_pModelRender->ForcedMaterialOverride(nullptr);
	}
