...
void c_menu::init()
{
	...
	c_tab* tab_first = main_window->add_tab(_w("Rage"));
	{
		...
		c_subtab* subtab_three = tab_first->add_subtab(_w("Anti-aim"));
		{
			...
			subtab_three->add_checkbox(_w("Slow-Motion"), _w("aa_fkwk"));
		...
		c_subtab* subtab_four = tab_first->add_subtab(_w("AA Regular"));
		{
			...
			subtab_four->add_checkbox(_w("Desync"), _w("aa_r_jitter"));
		...
		//	subtab_four->add_slider(_w("Desync swap"), _w("aa_r_jitter_speed"), 0, 64, _w("%d ° per tick"));
