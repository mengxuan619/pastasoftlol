...
void visuals::aa_indicator()
{
	...
	float delta = get_max_desync_delta2();
	float ptc = (int)ceilf((delta / 58.f) * 255.f);
	int green = (int)ptc;
	std::string text2;
	text2 = green;

//	g_Render.String(g_Options.antiaim.legitdesync.IndicatorX, g_Options.antiaim.legitdesync.IndicatorY + 10, CD3DFontFlags::CD3DFONT_DROPSHADOW, Color(0, green, 0), indicatorFont, "DESYNC");

//	render::get().text(Vector2D(10, size.Height / 2.f + 28.f), "FAKE " + text2, get_col(vars.aa.indicator_col.get<uintptr_t>()), fonts::lby, c_font::left_aligned | c_font::centered_y | c_font::drop_shadow);



	//render::get().text( Vector2D( player.top.x - player.width - 3, player.top.y + player.height - height - 5 ), tfm::format( d, player.health ), Color( 255, 255, 255, static_cast< int >( player.alpha ) ), fonts::esp_small, c_font::centered_x | c_font::centered_y | c_font::drop_shadow );
	float fake_strength = get_max_desync_delta2();
	if (fake_strength >= 58.f) //Color( 10, 240, 10, static_cast< int >( player.alpha ) )

		//render::get().text(Vector2D(10, size.Height / 2.f + 28.f), text2, Color(255, 255, 255, static_cast<int>(player.alpha)), fonts::esp_small, c_font::centered_x | c_font::centered_y | c_font::drop_shadow);
		render::get().text(Vector2D(10, size.Height / 2.f + 28.f), tfm::format("----Desync") + text2, Color(10, 240, 10, static_cast<int>(255)), fonts::esp_small, c_font::centered_x | c_font::centered_y | c_font::drop_shadow);

	//	D::Textf(10, ys - 80, Color(10, 240, 10), F::Watermark, "Desync");
	else if (fake_strength >= 45.f)
	//	D::Textf(10, ys - 80, Color(255, 250, 0), F::Watermark, "Desync");
		render::get().text(Vector2D(10, size.Height / 2.f + 28.f), tfm::format("----Desync") + text2, Color(255, 255, 0, static_cast<int>(255)), fonts::esp_small, c_font::centered_x | c_font::centered_y | c_font::drop_shadow);
	else if (fake_strength >= 35.f)
		render::get().text(Vector2D(10, size.Height / 2.f + 28.f), tfm::format("----Desync") + text2, Color(255, 150, 0, static_cast<int>(255)), fonts::esp_small, c_font::centered_x | c_font::centered_y | c_font::drop_shadow);
		//D::Textf(10, ys - 80, Color(255, 150, 0), F::Watermark, "Desync");
	else
		render::get().text(Vector2D(10, size.Height / 2.f + 28.f), tfm::format("----Desync") + text2, Color(255, 0, 0, static_cast<int>(255)), fonts::esp_small, c_font::centered_x | c_font::centered_y | c_font::drop_shadow);
	//	D::Textf(10, ys - 80, Color(255, 0, 0), F::Watermark, "Desync");
