...
{
	if ( !inventory ) // 8B 3D ? ? ? ? 85 FF 0F 84 ? ? ? ? BA
		inventory = **reinterpret_cast< CPlayerInventory*** >(sig("client_panorama.dll", "8B 3D ? ? ? ? 85 FF 0F 84 ? ? ? ? BA") + 0x2); //sig( "client_panorama.dll", "8B 0D ? ? ? ? 85 C9 74 18 51" ) + 0x2 );
